...
for Command in "$@"; do
    case $Command in
        import-files | reset)
            ID_Container_FILE_PROCESSOR=`docker ps | grep "test_${Runtime_Dirname##*/}_file_processor" | awk '{ print $1 }'`
            ID_Container_DB=`docker ps | grep "test_${Runtime_Dirname##*/}_db" | awk '{ print $1 }'`
